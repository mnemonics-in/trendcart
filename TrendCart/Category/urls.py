"""
URL configuration for TrendCart project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/5.0/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path
from Category import views
app_name="Category"
urlpatterns = [
    path('',views.addcategory, name="addcategory"),
    path('/viewmaincategory',views.viewmaincategory,name="viewmaincategory"),
    path('/deletemaincategory',views.deletemaincategory,name="deletemaincategory"),
    path('/delmaincategory/<int:p>', views.delmaincategory, name="delmaincategory"),
    path('/viewupdatemaincategory', views.viewupdatemaincategory, name="viewupdatemaincategory"),
    path('/updatemaincategory/<int:p>', views.updatemaincategory, name="updatemaincategory"),
    path('/updatemaincategory/updaterecord/<int:p>',views.updaterecord, name='updaterecord'),
    path('/addsubcategory', views.addsubcategory, name="addsubcategory"),
    path('/viewsubcategory', views.viewsubcategory, name="viewsubcategory"),
    path('/deletesubcategory', views.deletesubcategory, name="deletesubcategory"),
    path('/delsubcategory/<int:p>', views.delsubcategory, name="delsubcategory"),
    path('/viewupdatesubcategory', views.viewupdatesubcategory, name="viewupdatesubcategory"),
    path('/updatesubcategory/<int:p>', views.updatesubcategory, name="updatesubcategory"),
    path('/updatesubcategory/updatesubrecord/<int:p>', views.updatesubrecord, name='updatesubrecord'),

]
